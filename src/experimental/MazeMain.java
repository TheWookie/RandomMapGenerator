...
public class MazeMain {
	 ...
	 * finish within a reasonable amount of time. This is implemented iteratively,
	 * because we do not know when it could potentially finish it is better not to
	 * assume that the client machine has enough memory to accommodate for
	 * excessive usage.
	 * 
	 * @param rows
	 * @param columns
	...
	/** Will initialize and print to screen an array with a random number of rows
	 * and a random number of columns. This can be overriden with user via cli.
	 * 
	 * @param args
	 *          Users can provide up to two parameters. If they provide no
	 *          parameters, both rows and columns will be randomly assigned. If
	 *          the user provides a single number, then both the rows and columns
	 *          will be assigned that number. If the user provides two numbers
	 *          then the first will determine rows and the second will determine
	 *          columns. */
